...
- 🤖 **AI 单词 / 句子解析** —— 点击词元获得结构化解析。三层策略自动选择最优引擎：
  - **端侧推理**（通过 `flutter_gemma` 的 Gemma / MediaPipe），当本地已安装 `.task` 模型时启用，完全离线。
...
在应用内打开 **设置 → AI**，将 AudioLens 指向一个 AI 后端：

- **端侧推理（隐私优先，推荐）** —— 导入本地 Gemma `.task` 模型，或在设置页按 URL 下载。安装后自动优先使用。（Web 不支持。）
...
- **字幕** —— `SubtitleParser` 处理 SRT 与 ASS/SSA（自动识别），将首行正文视为英文、其余视为中文，并把英文分词为可点击的 `SubtitleToken`。
- **AI 解析**（`features/ai`）—— `AnalysisService.analyzeSubtitleSelection` 通过三层兜底选择模式；任何服务失败都会优雅降级为离线结果，而非抛异常。
